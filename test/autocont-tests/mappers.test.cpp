...
struct fake_solver {
  ...
  template <typename... Args> explicit fake_solver(Args...) {}

  template <typename DS, nld::Vector Reals, typename T = std::tuple<>>
  static Reals end_solution(const DS &, const constant_step_parameters &,
                            const Reals &variables, T &&args = no_arguments()) {
    return variables;
  }

  template <typename DS, nld::Vector Reals, typename T = std::tuple<>>
  static nld::matrix_xd solution(const DS &, const constant_step_parameters &,
                                 const Reals &, T &&args = no_arguments()) {
    nld::matrix_xd m(2, 2);
    m << 0, 0, 3, 0;
    return m;
  }
...
TEST_CASE("solution mapper just propagate second argument",
          "[nld::solution()]") {
...
TEST_CASE("mean_amplitude mapper returns half swing from solver",
          "[nld::mean_amplitude()]") {
  nld::vector_xd data(4);
  data << 1.0, 2.0, 3.0, 4.0;

  auto periodic = nld::periodic<fake_solver>(
      nld::autonomous(fn), nld::periodic_parameters_constant{1, 100});
