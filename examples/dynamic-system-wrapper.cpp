...
vector_xdd ABC_reaction(const vector_xdd& u, dual T, dual D)
{
    ...
    dual du_1 = - u(1) + D * (1 - u(0)) * exp(u(2)) + D * sigma * u(1) * exp(u(2));
    dual du_2 = - u(2) * betta * u(2) + D * B * (1 - u(0)) * exp(u(2)) + D * B * sigma * alpha * u(1) * exp(u(2));
...
vector_xdd ABC_reaction_0(const vector_xdd& u, dual D)
{
    ...
    du(2) = - u(2) * betta * u(2) + D * B * (1 - u(0)) * exp(u(2)) + D * B * sigma * alpha * u(1) * exp(u(2));
...
int main() {
    ofstream fs("/Volumes/Data/dev/nonlinear-dynamic-phd/src/3rd-year-report/data/stuff.csv");
    fs << 'x' << ';' << 'y' << endl;

    continuation_parameters params(
        newton_parameters(25, 0.00001), 0.8, 0.001, 0.001, direction::forward);
    ...
    u0 << 0.0, 0.0;
    dual lambda0 = 1.0;
    auto jacobian_hopf = autodiff::forward::jacobian(simple_system_2, wrt(u0), at(u0, lambda0));
    ...
    for (auto [v, A] : arc_length(bvp, params, us, vs, concat(solution(), mean_amplitude(0)))) {
        std::cout << v[3] << ';' << A << endl;
        fs << v[3] << ';' << v[0] << endl;
    }
}
