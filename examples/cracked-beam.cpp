...
struct clampted_hinged_beam_bc : boundary_condition {
    explicit clampted_hinged_beam_bc(double l) : length(l) {}

    auto value() const {
        return [l = length](auto x) -> adnum { return x * x * (x - l); };
    }
...
struct cracked_flexible_beam_dynamic_system final {
    ...
    void calculate_all_terms() {
        ...
        tensor_expression_calculator<gauss_kronrod21> calculator(
            integration_options{1.0e-4, 1.0e-4, 300});
        ...
        auto domain = segment{0.0, l};
        auto bd = basis_definition{40, {0.0, l}};
        ...
        auto kinetic_energy_no_crack = integral(rho * A * u * u, domain);
        auto potential_energy_no_crack =
            integral(E * I * laplacian(u) * laplacian(u), domain);
        auto epnc = solve_eigenvalue_problem<gauss_kronrod21>(
        ...
        auto Unc = eigenfunctions(u, std::move(eigenvectors_no_crack), dofs);

        auto Wnc = integral(Unc * delta_function(load.position), domain);
        ...
        auto Q = scalar_function(
            [=](adnum x) -> adnum { return 1.0 / (1.0 + L(x)); });
        auto kinetic_energy_crack = integral(rho * A * u * u, domain);
        auto potential_energy_crack =
            integral(E * I * Q * laplacian(u) * laplacian(u), domain);
        auto epc = solve_eigenvalue_problem<gauss_kronrod21>(
            kinetic_energy_crack, potential_energy_crack,
            integration_options{1.0e-3, 1.0e-3, 500});
        auto efc = epc.eigenvalues.cwiseSqrt();
        decltype(auto) eigenvectors_crack = sqrt(rho * A) * epc.eigenvectors;

        auto Uc = eigenfunctions(u, std::move(eigenvectors_crack), dofs);

        auto Wc = integral(Uc * delta_function(load.position), domain);
        auto [Fc0] = calculator.calculate(Wc);
        auto Tc = integral(rho * A * Uc * Uc, domain);
        auto [Mc] = calculator.calculate(Tc);
        Eigen::Map<nld::matrix_xd> massc(Mc.data(), dofs, dofs);
        nld::matrix_xd Mcinv_matrix = massc.inverse();
        auto nl_term_crack = integral(
            integral(diff(Uc, autodiff::wrt(x)) * diff(Uc, autodiff::wrt(x)),
                     domain) *
