...
vector_xdd NLTVA(const vector_xdd& y, dual t, dual Omega)
{
    ...
    auto k1 = 1.0;
    auto knl1 = 1.0;
    auto k2u = 8.0 * epsilon * (16.0 + 23.0 * epsilon + 9.0 * epsilon * epsilon + (4.0 + 2.0 * epsilon) * sqrt(4.0 + 3.0 * epsilon));
    auto k2l = 3.0 * (1.0 + epsilon) * (1.0 + epsilon) * (64.0 + 80.0 * epsilon + 27.0 * epsilon * epsilon);
    auto k2 = k2u / k2l;
    auto knl2 = 2.0 * epsilon * epsilon * knl1 / (1.0 + 4 * epsilon);
    auto caux = (k2 * m1 * epsilon * (8.0 + 9.0 * epsilon - 4.0 * sqrt(4.0 + 3.0 * epsilon))) / 4.0 / (1.0 + epsilon);
    auto c2 = sqrt(caux);
    ...
    dy[0] = y[2];
    dy[1] = y[3];
    dy[2] = -c1 * y[2] - k1 * y[0] - knl1 * y[0] * y[0] * y[0] - c2 * (y[2] - y[3])
        - k2 * (y[0] - y[1]) - knl2 * (y[0] - y[1]) * (y[0] - y[1]) * (y[0] - y[1]) + f0 * cos(t * 2.0 * PI);
    dy[3] = -c2 * (y[3] - y[2]) - k2 * (y[1] - y[0]) - knl2 * (y[1] - y[0]) * (y[1] - y[0]) * (y[1] - y[0]);
...
//     std::cout << "Start\n";
//     vector_xdd u1(5);
//     for (auto [solution, monodromy, amplitude] : arc_length(bvp, params, u0, v0, concat(solution(), monodromy(), mean_amplitude(0)))) {
...
//         for (i = 0; i < ev.size(); i++) {
//             is_saddle_node = (std::abs(ev(i).real() - 1.0) < 1.0e-3) && (std::abs(ev(i).imag()) < 1.0e-3);
//             if (is_saddle_node)
//                 break;
//         }
...
//     // auto bvp2 = periodic<runge_kutta_4>(non_autonomous(NLTVA), dimension(4), ip); 
...
//     // nld::vector_xdd tangential = nld::vector_xdd::Zero(5);
//     // tangential(4) = 1.0;

//     // for (auto value : arc_length(homotopy, params, known, tangential, [](const auto& v){ return v; })) {
//     //     auto kappa = value(4);
...
//     // for (auto [p, v] : arc_length(bvp, params, unknown, v0, half_swing(NLTVA, constant_step_parameters{0.0, 1.0, 200}, 0))) {
//     //     std::cout << p << ';' << v << endl;
//     //     fs << p << ';' << v << endl;
//     // }

//     // vector_xdd xo = vector_xd::Zero(2);

//     // runge_kutta_4 rk4(ode_p, constant_step_parameters{ 0.0, 1.0, 10 });
//     // runge_kutta_4 rk4_2(ode_p2, constant_step_parameters{ 0.0, 2.0 * PI, 10 });

//     // std::cout << "Solution 0:1\n" << rk4.solution(xo) << std::endl;
//     // std::cout << "Solution 0:2PI\n" << rk4_2.solution(xo) << std::endl;
// }
