...
template <OdeSolver S, typename Ds> struct saddle_node;
...
struct saddle_node final : nld::problem<Ds> {
  ...
  using solver_t = S;
  using integration_parameters_t = typename solver_t::integration_parameters_t;
  ...
  explicit saddle_node(Ds &&ds, nld::periodic_parameters params)
  ...
  template <typename Vector> auto ode(const Vector &variables) const {
    using namespace std::placeholders;

    auto parameters_on_plane = variables.tail(internal::parameters);
    auto ds = std::bind(this->function.function, _1, _2, parameters_on_plane);
  ...
  /// @param variables First N are initial conditions for ODE, next N are eigen
  /// values, last are two parameters.
  ...
  operator()(Vector &variables) const {
    ...
    auto dyTdy0 =
        autodiff::forward::jacobian(solution_at_end, wrt(state), at(variables));
  ...
  auto jacobian(Wrt &&wrt, At &&at, Result &v) const -> matrix_xd {
    ...
    vector_xd phi = variables.segment(states, states).template cast<double>();
    ...
    J.topLeftCorner(states, states) -= nld::matrix_xd::Identity(states, states);
    J.block(states, states, states, states) = J.topLeftCorner(states, states);
    J.block(states, 0, states, states) = dJphidy0.topLeftCorner(states, states);
...
private:
  auto end_solution() const {
    return [this]<typename Vector>(const Vector &variables) {
      ...
      return solver_t::end_solution(this->function, parameters, state, args);
    };
  }
...
template <OdeSolver S, typename Ds>
auto saddle_node(Ds dynamic_system, periodic_parameters parameters) {
