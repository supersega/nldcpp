...
#include <nld/autocont/equilibrium.hpp>
#include <nld/autocont/homotopy.hpp>
...
first_guess evaluate_firs_guess(const equilibrium<Fn>& eq, dimension size) {
    ...
    convex_homotopy homotopy(std::move(system), std::move(fixed_point_homotopy));
    ...
    continuation_parameters params(
       newton_parameters(25, 0.00009), 8.5, 0.002, 0.0025, direction::forward);

    for (auto value : nld::internal::arc_length_raw(std::move(homotopy), params, known, tangential, solution())) {
...
//         return first_guess { nld::vector_xdd::Zero(unknowns), std::move(tangential) };
//     }
//     else {
//         const auto dimension = pd.dimension();
//         const auto unknowns = dimension + 1;
