...
constexpr auto concat(M&&... map) noexcept {
    return [... map = std::forward<M>(map)](const auto& fn, const auto& v) noexcept {
...
constexpr auto point2d(nld::index n, nld::index m) noexcept {
    return [n, m](const auto& fn, const auto& v) noexcept { return std::tuple(v[n], v[m]); };
}
...
constexpr auto mean_amplitude(nld::index coordinate) noexcept {
    ...
    (const P& periodic, const auto& variables) noexcept {
        auto [initial, args] = integration_arguments(variables, periodic);

        return mean<typename P::solver_t>(
...
constexpr auto half_swing(nld::index coordinate) noexcept {
    return [coordinate]<typename P>
    (const P& periodic, const auto& variables) noexcept {
        return std::tuple(variables(variables.size() - 1), mean_amplitude(coordinate)(periodic, variables));
    };
}
...
constexpr auto monodromy() noexcept {
    return []<typename P>
    (const P& periodic, auto variables) noexcept {
        const auto dimension = variables.size() - periodic.non_state_variables();

        auto jacobian = periodic.jacobian(wrt(variables.head(dimension)), at(variables));
