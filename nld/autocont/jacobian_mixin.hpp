...
/// @brief Class to inject jacobian functions for nonlinear functions using operator().
...
struct jacobian_mixin : nld::utils::crtp<F> {
    ...
    auto jacobian(Wrt&& wrt, At&& at) const {
        return autodiff::forward::jacobian(this->derived(), wrt, at);
    }
};
} // end namespace nld::wrappers
