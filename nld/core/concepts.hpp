...
template<typename S>
concept Scalar = requires(S s) {
    S(std::declval<double>());
};

static_assert(Scalar<archetypes::scalar>, "Please update scalar archetype to satisfy Scalar concept");
...
static_assert(Vector<archetypes::vector>, "Please update vector archetype to satisfy Vector concept");
...
/// @details OdeSolver should provide solution static function and end_solution function.
/// See arguments bellow to understand signature.
/// solution - assumed to return whole solution on interval stored in Matrix.
/// end_solution - assumed to return end solution on interval stored in Vector.
template<typename S>
concept OdeSolver = requires(S s) {
    typename S::integration_parameters_t;
    { S::solution(std::declval<archetypes::ode>(), std::declval<typename S::integration_parameters_t>(), std::declval<archetypes::vector>()) } -> Matrix;
    { S::end_solution(std::declval<archetypes::ode>(), std::declval<typename S::integration_parameters_t>(), std::declval<archetypes::vector>()) } -> Vector;
};
...
template<typename F>
concept FunctionWithPreviousStepSolution = requires(F f) {
    f.set_previous_solution(std::declval<utils::any_type>());
};
