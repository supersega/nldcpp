...
struct crtp {
    ...
    const T& derived() const {
        return const_cast<crtp<T>&>(*this).derived();
    }
protected:
    crtp() = default;
};
...
auto tensor_view(const matrix_generic<Real, Rows, Cols>& matrix)
{
    if constexpr (Cols == 1 || Rows == 1)
        return Eigen::TensorMap<Eigen::Tensor<const Real, 1>>(matrix.data(), matrix.size());
    else
        return Eigen::TensorMap<Eigen::Tensor<const Real, 2>>(matrix.data(), matrix.rows(), matrix.cols());
}
}
