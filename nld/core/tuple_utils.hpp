...
template <typename T>
auto &get_element(T &t, std::size_t i) {
...
auto count(T &&t) -> std::size_t {
    std::size_t n = 0;
    for_each(t, [&n](auto &&e) { n += detail::get_size(e); });
    return n;
}
...
auto vector_to_tuple(const V &v, T &t) -> void {
    ...
    for_each(t, [&](auto &&e) {
        for (Eigen::Index i = 0; i < detail::get_size(e); i++)
            detail::get_element(e, i) = v(i + n);
...
auto tuple_to_vector(const T &t, V &&v) -> void {
    ...
    for_each(t, [&](auto &&e) {
        for (Eigen::Index i = 0; i < detail::get_size(e); i++)
            v(i + n) = static_cast<double>(detail::get_element(e, i));
...
auto make_index_sequence_impl() {
    if constexpr (End == Start)
        ...
        return make_index_sequence_impl<Start, End - 1, End - 1, Is...>();
}

/// Create index sequence on [Start, End)
template <std::size_t Start, std::size_t End>
using make_index_sequence =
    std::decay_t<decltype(make_index_sequence_impl<Start, End>())>;

/// Create tuple view using index sequence
template <class Tuple, size_t... Is>
constexpr auto view_impl(Tuple &&t, std::index_sequence<Is...>) {
