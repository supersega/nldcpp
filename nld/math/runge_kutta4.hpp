...
/// TODO: Make possible to pass arguments into ode solvers to make wrapping elegant
...
V runge_kutta_4_step(const DS& ode, S start, S end, const V& initial_conditions, T&& args);
} // end namespace detail

/// @brief The class to approximate ODE solution using Runge-Kutta 4 order method.
/// @details This class implements standart Runge-Kutta 4 method with constant step.
...
struct runge_kutta_4 final {
    ...
    /// @details This function calculate and store whole solution on given interval.
    ...
    static auto solution(const DS& ode, integration_parameters_t parameters, const V& initial, T&& args = no_arguments()) -> nld::matrix_xd {
        ...
        for (nld::index i = 0; i < intervals; i++) {
            solution_at_end = nld::math::detail::runge_kutta_4_step(ode, start, start + step, solution_at_end, args);
    ...
    /// @details This function calculate and store end solution on given interval.
    ...
    template<typename DS, Vector V, typename T = std::tuple<>>
    static V end_solution(const DS& ode, integration_parameters_t parameters, const V& initial, T&& args = no_arguments()) {
        auto [start, end, intervals] = parameters;
        auto step = (end - start) / intervals;
        ...
        for (nld::index i = 0u; i < intervals; i++) {
            solution_at_end = nld::math::detail::runge_kutta_4_step(ode, interval_start, interval_start + step, solution_at_end, args);
            interval_start += step;
        }
...
namespace detail {
/// @brief Evaluate ode function at given point, time, with additional arguments.
...
template<typename DS, Scalar S, Vector V, typename T>
V runge_kutta_4_step(const DS& ode, S start, S end, const V& initial, T&& args) {
    auto solution_at_end = initial;
