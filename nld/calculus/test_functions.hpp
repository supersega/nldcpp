...
struct test_functions final {
    ...
    explicit test_functions(const Space& space, Basis... basis) : space(space), bases(std::make_tuple(basis...)), size(0)
    {
        nld::utils::for_each(bases, [this](auto&& basis) { this->size += basis.count(); });
    }
    ...
    auto value(nld::index i) const requires(dimension == 1) {
    ...
    auto subdomain(nld::index i) const -> std::optional<nld::segment> requires(dimension == 1) {
...
test_functions(const Space& space, Basis... basis) -> test_functions<Space, std::tuple<Basis...>>;
}