...
auto eval_impl(const E &mul, const D &domain, integration_options options)
...
auto eval_impl(const nld::dirac_shift<E, F> &shift, const D &domain,
               [[maybe_unused]] integration_options options) {
    ...
    for (nld::index i = 0; i < ni; i++)
        for (nld::index j = 0; j < nj; j++) {
            if constexpr (SubdomainDefined<E>) {
                ...
                subdomain =
                    subdomain ? domain.intersect(*subdomain) : std::nullopt;
                result(j, i) =
...
template <typename I, typename E, typename D>
auto eval_impl(const E &expr, const D &domain, integration_options options)
...
template <typename I, typename E, typename D>
auto eval(const E &expr, const D &domain, integration_options options)
