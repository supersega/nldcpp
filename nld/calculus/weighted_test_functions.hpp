...
struct weighted_test_functions final {
    ...
    /// @param definition The basis definition (number of basis functions and
    /// interval) in dimension.
    ...
    auto value(nld::index i) const {
        return
            [basis = underlying_test_functions, bc = boundary_condition,
             i](auto x) -> adnum { return basis.value(i)(x) * bc.value()(x); };
    }
...
template <typename Space, typename Basis, typename Bc>
weighted_test_functions(test_functions<Space, Basis> tf, Bc bc)
    -> weighted_test_functions<test_functions<Space, Basis>, Bc>;
} // namespace nld
