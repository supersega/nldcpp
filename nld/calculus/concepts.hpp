...
concept ThreeDimensionScalarFunction = requires(F f) {
    { f(std::declval<adnum>(), std::declval<adnum>(), std::declval<adnum>()) } -> std::same_as<adnum>;
};

/// @brief Three scalar function supported for calculus.
/// @tparam F callable function.
template<typename F>
concept ScalarFunction = OneDimensionScalarFunction<F> || TwoDimensionScalarFunction<F> || ThreeDimensionScalarFunction<F>;
...
/// @tparam T type.
template<typename T>
concept BasisWithFunctionsOnSubdomains = std::is_base_of_v<basis, T> && requires(T b) {
...
template<typename Space, typename Basis>
struct is_subdomain_defined<test_functions<Space, Basis>> : std::conditional_t<test_functions<Space, Basis>::with_subdomains, std::true_type, std::false_type> { };

template<typename T, typename B>
struct is_subdomain_defined<weighted_test_functions<T, B>> : std::conditional_t<std::remove_reference_t<T>::with_subdomains, std::true_type, std::false_type> { };

template<typename L, typename R>
struct is_subdomain_defined<tensor_mul<L, R>> : std::conditional_t<is_subdomain_defined<std::remove_reference_t<R>>::value, std::true_type, std::false_type> { };

template<typename L, typename F>
struct is_subdomain_defined<tensor_mul<L, nld::scalar_function<F>>> : std::conditional_t<std::is_arithmetic<std::remove_reference_t<L>>::value, std::true_type, std::false_type> { };

template<typename E, typename W>
struct is_subdomain_defined<derivative<E, W>> : std::conditional_t<is_subdomain_defined<std::remove_reference_t<E>>::value, std::true_type, std::false_type> { };
...
/// @tparam T type.
template<typename L, typename R>
concept TensorMulOperands = (TensorExpression<L> && TensorExpression<R>) || (Constant<L> && TensorExpression<R>) || (TensorExpression<L> && Constant<R>);

/// @brief TensorExpression concept to define expressions.
/// @tparam T type.
template<typename T>
concept ScalarFunctionExpression = nld::internal::is_scalar_function_expression<T>::value;
...
concept Integrator = requires(S s) {
    ...
    { S::integrate(std::declval<archetypes::scalar_function>(), std::declval<typename S::integration_parameters_t>(), std::declval<archetypes::domain>()) } -> Scalar;
};
}
