...
struct tensor_expression_calculator final {
    using integration_options = typename I::integration_options;
    explicit tensor_expression_calculator(
        integration_options opt = integration_options{})
        : options(opt) {}
...
private:
    ...
    auto calculate(const nld::add<L, R> &add, const D &domain,
                   const T &tuple) const {
    ...
    template <typename L, typename R, typename D, typename T>
    auto calculate(const nld::mul<L, R> &mul, const D &domain,
                   const T &tuple) const {
        auto left = calculate(mul.left);
        auto right = calculate(mul.right, domain, std::make_tuple());

        static_assert(std::tuple_size_v<decltype(left)> == 1 &&
                          std::tuple_size_v<decltype(right)> == 1,
                      "It is not possible now to use complicated expression in "
                      "internal integral");
    ...
    auto calculate(const E &expr, const D &domain, const T &tuple) const {
        return std::tuple_cat(
            tuple, std::tuple(nld::internal::eval<I>(expr, domain, options)));
    }

    template <typename E, typename F, typename D, typename T>
    auto calculate(const dirac_shift<E, F> &mul, const D &domain,
                   const T &tuple) const {
        return std::tuple_cat(
            tuple, std::tuple(nld::internal::eval<I>(mul, domain, options)));
    }
