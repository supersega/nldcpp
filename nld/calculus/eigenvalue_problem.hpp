...
auto solve_eigenvalue_problem(
    K &&kinetic_energy, P &&potential_energy,
    typename I::integration_options options = typename I::integration_options{})
    -> eigenvalue_problem_solution {
    ...
    Eigen::Map<nld::matrix_xd> mass(mass_tensor.data(), dimensions[0],
                                    dimensions[1]);
    Eigen::Map<nld::matrix_xd> stiffness(stiffness_tensor.data(), dimensions[0],
                                         dimensions[1]);

    Eigen::GeneralizedSelfAdjointEigenSolver<nld::matrix_xd> es(stiffness,
                                                                mass);
