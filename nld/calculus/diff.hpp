...
struct derivative final {
    ...
    explicit derivative(E&& e, W&& w) : expression(std::forward<E>(e)), wrt(std::forward<W>(w)) { }

    /// @brief Get derivatives for i-th test function.
    /// @param i coordinate number.
    auto value(std::tuple<nld::index> i) const requires(expression_t::dimension == 1) {
        return [*this, i](auto x) -> adnum {
            const auto& sp = this->expression.get_space();
            auto& xcoord = sp.coords()[0];
            xcoord = x;
            return autodiff::val(autodiff::derivative(expression.value(i), std::move(wrt), at(xcoord))); 
        };
    }
